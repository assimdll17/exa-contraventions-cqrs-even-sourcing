...
public class InfractionServiceHandler {
   ...
   /* @EventHandler
    ...
    @QueryHandler
    public List<CreateInfractionRequestDTO> on(GetInfractionsByVehicle query) {
        List<Infraction> infractions = infractionRepository.findByMatriculeEquals(query.getId());
        List<CreateInfractionRequestDTO> infractionResponseDTOS = new ArrayList<>();
        for (Infraction infraction: infractions) {
            CreateInfractionRequestDTO infractionResponseDTO = new CreateInfractionRequestDTO();
